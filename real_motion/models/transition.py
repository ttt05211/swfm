"""Window-compatible OccFM transition model with anchor and history-context conditioning."""
import torch
...
class MotionWindowFlowMatching(FLOW_MATCHING_DOWN_X4_DiT):
    ...
    P0-F4 keeps the expensive future transition local (20x20) but restores
    surrounding scene context through a cheap 40x40 full-history branch.  The
    context branch is a single temporal mean + stride-2 3x3 projection, not a
    second world model.  Both new anchor/context projections are zero-initialized
    so the inherited OccFM transition is unchanged at initialization.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Larger historical context (normally 40x40) is temporally summarized
        # then projected to an approximately 20x20 feature map. It is resized to
        # each U-Net scale before attention conditioning.
    ...
    def _context_base(self, history_context, batch_size, device, dtype):
        ...
        # Local 20x20 history already carries explicit time ordering. The larger
        # context branch supplies surrounding scene layout/agent context only.
