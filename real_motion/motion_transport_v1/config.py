...
def validate_config(c):
    ...
    max_hours=_get(c,'training.max_hours')
    if max_hours is not None and float(max_hours)<=0:raise ValueError('training.max_hours must be positive or null')
    gpus=_get(c,'training.gpus')
    if gpus is not None and int(gpus) not in (1,2):raise ValueError('MT-V1 currently supports 1 or 2 training GPUs')
