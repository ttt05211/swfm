...
def _sample_grid_stable_at_lattice(tmpl,norm_chunk,u_chunk,dims,jitter_voxels):
    """Exact grid_sample forward with a symmetric gradient only at lattice centers.

    PyTorch 2.6 chooses a one-sided derivative at exact trilinear knots. With CE
    probabilities near epsilon this creates a version-sensitive first-step spike.
    The hard renderer is unchanged; for the training-only soft surrogate we keep
    the exact forward value and replace only the knot gradient by the mean of four
    symmetric XY sub-voxel samples. Away from knots the native gradient is used.
    """
...
def _torch_rigid_inverse_sample(source,delta,horizon_s,future_pose,flat_indices,*,grid,template_cache,query_chunk,center_gradient_jitter_voxels=1e-3):
...
def render_soft_ordered(causal,decomp,deltas,selected_source_ids,*,grid,class_count=18,halo_voxels=2,query_chunk=65536,full_grid_reference=False,center_gradient_jitter_voxels=1e-3):
