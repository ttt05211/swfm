...
def _torch_rigid_inverse_sample(source,delta,horizon_s,future_pose,flat_indices,*,grid,template_cache,query_chunk):
    ...
    for start in range(0,len(flat),max(1,int(query_chunk))):
        g=norm[start:start+query_chunk].reshape(1,1,1,-1,3);outs.append(F.grid_sample(tmpl,g,mode='bilinear',padding_mode='zeros',align_corners=False).reshape(-1))
