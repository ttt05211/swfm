...
def _validity_audit_update(audit,targets,moving_support=None,fi=None):
    valid=np.asarray(targets.future_valid,bool);sem=np.asarray(targets.future_semantics);obs=np.asarray(targets.future_observed,bool) if targets.future_observed is not None else np.zeros_like(valid,bool);audit['valid_voxels']+=int(valid.sum());audit['invalid_voxels']+=int((~valid).sum());audit['valid_observed']+=int((valid&obs).sum());audit['valid_unobserved']+=int((valid&~obs).sum());audit['valid_nonfree_observed']+=int((valid&obs&(sem!=17)).sum());audit['valid_nonfree_unobserved']+=int((valid&~obs&(sem!=17)).sum())
    if moving_support is not None and fi is not None:
