...
REPORT={1.0:1,2.0:3,3.0:5};GROUP_NAMES=('observed-moving','dormant-to-moving','stationary','stopping','no-causal-source')
...
def _groups(source,window,causal,decomp,targets,fi,h,moving_records,grid):
    ...
    for r in moving_records:
        if str(r['instance_token']) in matched:continue
        out['no-causal-source']|=rasterize_oriented_box(box3d_from_dict(r['box0_future_ego']),gs,BOX_MARGIN_M)|rasterize_oriented_box(box3d_from_dict(r['boxh_future_ego']),gs,BOX_MARGIN_M)
    return out
...
def _report(st):
    oh={h:st['overall'][h].compute() for h in REPORT_HORIZONS_S};sh={h:st['stationary'][h].compute() for h in REPORT_HORIZONS_S};gh={n:{h:st['groups'][n][h].compute() for h in REPORT_HORIZONS_S} for n in GROUP_NAMES};tp,fp,fn=st['tp'],st['fp'],st['fn'];return {'overall':{'mIoU':float(np.nanmean([oh[h]['mIoU'] for h in REPORT_HORIZONS_S])),'per_horizon':oh},'moving':st['moving'].compute(),'stationary_movable':{'mIoU':_mean(sh),'per_horizon':sh},'motion_groups':{n:{'mIoU':_mean(gh[n]),'per_horizon':gh[n]} for n in GROUP_NAMES},'dynamic_precision':tp/max(1,tp+fp),'dynamic_recall':tp/max(1,tp+fn),'dynamic_tp':tp,'dynamic_fp':fp,'dynamic_fn':fn}
