...
def load_official_vae(upstream_root,checkpoint,device="cuda",config_rel="tools/cfgs/occfm_vae.yaml"):
    ...
    model.recover_training(checkpoint);model.eval().to(device);model.requires_grad_(False);return model,cfg


def load_official_wm(upstream_root,checkpoint,device="cuda",config_rel="tools/cfgs/occfm_fut.yaml"):
...
def _seeded_eps_like(mu,seed,logical_batch,frames_per_sample):
    if seed is None:return torch.randn_like(mu)
    if isinstance(seed,Sequence) and not isinstance(seed,(str,bytes)):
        seeds=[int(s) for s in seed]
        if len(seeds)!=int(logical_batch):raise ValueError(f'per-sample seed length {len(seeds)} != logical batch {logical_batch}')
        expected=int(logical_batch)*int(frames_per_sample)
        if mu.shape[0]!=expected:raise ValueError(f'latent batch {mu.shape[0]} incompatible with {logical_batch}x{frames_per_sample}')
        chunks=[]
        for i,s in enumerate(seeds):
            gen=torch.Generator(device=mu.device);gen.manual_seed(s);part=mu[i*frames_per_sample:(i+1)*frames_per_sample];chunks.append(torch.randn(part.shape,device=mu.device,dtype=mu.dtype,generator=gen))
        return torch.cat(chunks,dim=0)
    gen=torch.Generator(device=mu.device);gen.manual_seed(int(seed));return torch.randn(mu.shape,device=mu.device,dtype=mu.dtype,generator=gen)
...
class OccFMVAEAdapter:
    ...
    def encode(self,semantic_occ,mode="sample",seed=None):
        ...
        if x.device!=self.device or x.dtype!=torch.long:x=x.to(device=self.device,dtype=torch.long,non_blocking=(x.device.type=='cpu' and x.is_pinned()))
        ...
        video=x.ndim==5;B=x.shape[0];F=x.shape[1] if video else 1;batch={"semantic_occ":x};old_e,old_enc=self.model.embedding.skip,self.model.encoder.skip
        try:self.model.embedding.skip=False;self.model.encoder.skip=False;batch=self.model.embedding(batch);batch=self.model.encoder(batch)
        finally:self.model.embedding.skip=old_e;self.model.encoder.skip=old_enc
        compressed=batch["compressed_features"];dim=compressed.shape[1]//2;mu=compressed[:,:dim];sigma=torch.exp(compressed[:,dim:]/2).to(compressed.dtype)
    ...
    def decode_logits(self,latent):
        ...
        B=z.shape[0];F=z.shape[1] if video else None;flat=z.reshape(-1,*z.shape[-3:]) if video else z;batch={"sampled_features":flat};old=self.model.decoder.skip
        try:self.model.decoder.skip=False;batch=self.model.decoder(batch)
        finally:self.model.decoder.skip=old
        decoded=batch["decoded_map"];height=self.model.embedding.height_num;cate=self.model.embedding.cate;decoded=decoded.reshape(decoded.shape[0],height,cate,decoded.shape[-2],decoded.shape[-1]);decoded=decoded.permute(0,3,4,1,2);template=self.model.embedding.class_embeds.weight.T.unsqueeze(0).detach();logits=torch.matmul(decoded,template)
    ...
    def empty_latent(self,shape_hwd=(200,200,16),free_label=17,mode="mean",seed=0):
        empty=torch.full(shape_hwd,int(free_label),device=self.device,dtype=torch.long);return self.encode(empty,mode=mode,seed=seed)
...
def run_frozen_occfm_forecast(wm,history_latent,future_reference_latent,trajectory=None,seed=0,hist_last=4):
    ...
    if int(hist_last)!=4:raise ValueError("official OccFM-fut release uses HIST_LAST=4")
    ...
    hist=hist.clone()
    if hist_last<hist.shape[0]:hist[:hist.shape[0]-int(hist_last)]=0
    x=torch.cat([hist,fut],dim=0).unsqueeze(0)
    if trajectory is None:raise ValueError("OccFM-fut 196 requires the official [12,2] GT ego trajectory")
    traj=torch.as_tensor(trajectory,device=device,dtype=x.dtype)
    if traj.shape!=(12,2):raise ValueError(f"OccFM-fut trajectory must be [12,2], got {tuple(traj.shape)}")
