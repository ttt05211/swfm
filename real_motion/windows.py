...
class WindowPlanner:
    def __init__(self,window_hw=(20,20),max_windows=8):
        ...
        if min(self.window_hw)<=0 or self.max_windows<=0: raise ValueError("invalid window configuration")
...
def crop_windows(x:torch.Tensor,plan:WindowPlan)->torch.Tensor:
    ...
    b=x.shape[0]; k=plan.origins.shape[1]; wh,ww=plan.window_hw; out=x.new_zeros((b,k,*x.shape[1:-2],wh,ww))
    for bi in range(b):
        for ki in range(k):
            if not bool(plan.valid[bi,ki]): continue
            y,x0=[int(v) for v in plan.origins[bi,ki]]; out[bi,ki]=x[bi,...,y:y+wh,x0:x0+ww]
    return out

def scatter_windows(windows:torch.Tensor,plan:WindowPlan,base:torch.Tensor|None=None,weight:torch.Tensor|None=None)->torch.Tensor:
    ...
    out=windows.new_zeros((b,*windows.shape[2:-2],h,w)); cnt=windows.new_zeros((b,*([1]*(windows.ndim-4)),h,w))
    if weight is not None and weight.shape[:2]!=(b,k): raise ValueError("weight batch/slot mismatch")
    for bi in range(b):
        for ki in range(k):
            if not bool(plan.valid[bi,ki]): continue
            y,x0=[int(v) for v in plan.origins[bi,ki]]; wwgt=1.0 if weight is None else weight[bi,ki]; out[bi,...,y:y+wh,x0:x0+ww]+=windows[bi,ki]*wwgt; cnt[bi,...,y:y+wh,x0:x0+ww]+=wwgt
...
def window_coverage(support:torch.Tensor,plan:WindowPlan)->torch.Tensor:
    ...
    if tuple(union.shape[-2:])!=plan.full_hw or union.shape[0]!=plan.valid.shape[0]: raise ValueError("support and plan shape mismatch")
    cover=torch.zeros_like(union); wh,ww=plan.window_hw
    for bi in range(union.shape[0]):
        for ki in range(plan.valid.shape[1]):
            if not bool(plan.valid[bi,ki]): continue
            y,x0=[int(v) for v in plan.origins[bi,ki]]; cover[bi,y:y+wh,x0:x0+ww]=True
    numer=(cover&union).flatten(1).sum(1).float(); denom=union.flatten(1).sum(1).float(); return torch.where(denom>0,numer/denom,torch.ones_like(denom))
