...
def build_motion_tube(kta_support: torch.Tensor,
    ...
    """Horizon-dependent dilation of a causal KTA BEV support.

    Args:
        kta_support: [F,H,W] or [B,F,H,W]
    """
    ...
    if not radii:
        raise ValueError("at least one tube radius is required")
    if len(radii) < f:
...
def downsample_support(mask: torch.Tensor, out_hw=(50, 50), extra_radius: int = 1) -> torch.Tensor:
    """Conservative max-pool downsampling from occupancy BEV to latent BEV."""
    ...
    if extra_radius:
        pooled = _dilate_2d(pooled, int(extra_radius))
...
def coverage_and_active_ratio(gt_moving_support: torch.Tensor,
    ...
    gt = gt_moving_support.bool()
    gen = generation_support.bool()
    if gt.shape != gen.shape:
        raise ValueError(f"shape mismatch: {gt.shape} vs {gen.shape}")
    inter = (gt & gen).sum(dtype=torch.float64)
    denom = gt.sum(dtype=torch.float64)
    coverage = (inter / denom).item() if denom.item() else 1.0
    active = gen.float().mean().item()
    return coverage, active


def per_horizon_stats(gt_moving_support: torch.Tensor,
                      generation_support: torch.Tensor):
    """Return coverage/active ratio separately for each horizon."""
    gt = gt_moving_support.bool()
    gen = generation_support.bool()
    if gt.shape != gen.shape or gt.ndim != 3:
        raise ValueError("expected aligned [F,H,W] tensors")
    rows = []
    for h in range(gt.shape[0]):
        cov, active = coverage_and_active_ratio(gt[h], gen[h])
        rows.append({"horizon_index": h, "coverage": cov, "active_ratio": active,
                     "gt_ratio": gt[h].float().mean().item()})
    return rows
