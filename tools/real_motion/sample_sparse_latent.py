#!/usr/bin/env python3
"""Sample sparse future moving latents and scatter them to a full latent canvas."""
import argparse,sys
...
def make_model(window):
    tr=MotionWindowFlowMatching(in_channels=16,out_channels=16,model_channels=128,channel_multi=[2,4],
        input_size=[window,window],trajectory_length=6,init_kernel_size=7,init_3d_conv_channels=64,
        attn_dim=32,temporal_attn_head=8,spatial_attn_head=8,prior_channels=32)
    return RealMotionWindowCFM(tr)

def main():
    p=argparse.ArgumentParser()
    p.add_argument("--cache",required=True); p.add_argument("--ckpt",required=True)
    p.add_argument("--output",required=True); p.add_argument("--empty-latent",required=True,
        help="torch tensor [C,50,50] = frozen VAE E(empty occupancy)")
    p.add_argument("--window",type=int,default=20); p.add_argument("--max-windows",type=int,default=8)
    ...
    dl=DataLoader(ds,batch_size=a.batch_size,shuffle=False,collate_fn=collate_real_motion)
        ...
        for batch in dl:
            ...
            if min_cov < a.min_window_coverage:
                msg=(f"future generation-support coverage {min_cov:.3f} < "
                     f"{a.min_window_coverage:.3f}; sampling would silently miss target regions")
            ...
            F=batch["static_future_latent"].shape[1]
            if not bool(valid.any()):
                base=empty.to(dev)[None,None].expand(B,F,-1,-1,-1).clone()
                outputs.append(base.cpu())
                continue
            def flat(x): return x.reshape(B*K,*x.shape[2:])[valid]
            fhist,fsta,fkta=map(flat,(hist,sta,kta))
            ...
            pred_valid=model.sample(fhist,tuple(fsta.shape[:2])+ (16,a.window,a.window),prior,trajectory=traj,window_origins=origins)
            # restore padded K slots for deterministic scatter
            F=pred_valid.shape[1]
            pred_pad=torch.zeros(B*K,F,16,a.window,a.window,device=dev,dtype=pred_valid.dtype)
            pred_pad[valid]=pred_valid
            pred_pad=pred_pad.reshape(B,K,F,16,a.window,a.window)
            base=empty.to(dev,dtype=pred_pad.dtype)[None,None].expand(B,F,-1,-1,-1)
            full=scatter_windows(pred_pad,plan,base=base)
            # Never treat window coverage as overwrite permission downstream.
            outputs.append(full.cpu())
    out=torch.cat(outputs,dim=0)
    Path(a.output).parent.mkdir(parents=True,exist_ok=True)
    torch.save({"future_moving_latent":out},a.output)
    print("saved",a.output,out.shape)
