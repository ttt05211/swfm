...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--cache",required=True); p.add_argument("--ckpt",required=True); p.add_argument("--output",required=True); p.add_argument("--empty-latent",required=True); p.add_argument("--window",type=int,default=20); p.add_argument("--max-windows",type=int,default=8); p.add_argument("--batch-size",type=int,default=1); a=p.parse_args(); dev="cuda" if torch.cuda.is_available() else "cpu"; ds=RealMotionCacheDataset(a.cache); dl=DataLoader(ds,batch_size=a.batch_size,shuffle=False,collate_fn=collate_real_motion); model=make_model(a.window).to(dev); ck=torch.load(a.ckpt,map_location="cpu",weights_only=False); model.load_state_dict(ck["state_dict"],strict=True); model.eval(); empty=torch.load(a.empty_latent,map_location="cpu",weights_only=False)
    ...
    with torch.no_grad():
        for batch in dl:
            ...
            pred_valid=model.sample(fhist,tuple(fsta.shape[:2])+(16,a.window,a.window),prior,trajectory=traj,window_origins=origins); F=pred_valid.shape[1]; pred_pad=torch.zeros(B*K,F,16,a.window,a.window,device=dev,dtype=pred_valid.dtype); pred_pad[valid]=pred_valid; pred_pad=pred_pad.reshape(B,K,F,16,a.window,a.window); base=empty.to(dev,dtype=pred_pad.dtype)[None,None].expand(B,F,-1,-1,-1); outputs.append(scatter_windows(pred_pad,plan,base=base).cpu())
