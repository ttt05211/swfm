#!/usr/bin/env python3
"""Evaluate v13 learned displacement through coherent rigid occupancy transport.

Unlike discarded v12, both training diagnostics and GT oracle operate on object
*displacement*.  A Strong source component keeps its observed shape offset; the
predicted/GT displacement is added to that component instead of aligning its
visible-voxel centroid to an annotation box center.
"""
...
PROTOCOL = "p0_f9_v13_learned_motion_transport_eval_v2"
...
def main():
    ...
    p.add_argument("--motion-cache", required=True); p.add_argument("--p0f9-cache", required=True)
    ...
    p.add_argument("--match-max-distance-m", type=float, default=4.0); p.add_argument("--max-windows", type=int, default=0); p.add_argument("--device", default="cuda")
    a = p.parse_args()
    ...
    if missing: raise RuntimeError(f"P0-F9 cache misses motion-val samples: {missing[:5]}")
    ...
    strong_cfg = StrongW2DetConfig(free_label=int(pcfg.free_label)); states = metric_states()
    ...
    for wi, rec in enumerate(records):
        sid = str(rec["sample_id"]); s = ds[sid_to_idx[sid]]; payload = safe._sample_payload(s, torch.device("cpu")); w = window_from_record(rec)
        ...
        if len(current) != int(rec["features"].shape[0]): raise RuntimeError(f"{sid}: reconstructed Strong source count differs from motion cache")
        got_classes = [int(c["class_id"]) for c in current]
        if got_classes != [int(x) for x in rec["source_class_id"].tolist()]: raise RuntimeError(f"{sid}: reconstructed Strong source ordering differs from motion cache")

        features = rec["features"].float().to(device)
        with torch.no_grad():
            out = model(features); pred_residual = out["residual_xy_m"].cpu().numpy(); exist_prob = torch.sigmoid(out["existence_logits"]).cpu().numpy()
        ...
        for horizon, hi in safe.REPORT.items():
            ...
            for i, comp in enumerate(current):
                ...
                # anchors are component-centroid KTA positions; adding the learned
                # displacement residual preserves the source-shape offset.
                ...
                if token is not None:
                    ...
                    if ann0 is not None and annh is not None:
                        # Critical v13 invariant: move the observed component by
                        # the GT object displacement; never align its centroid to
                        # the future annotation center.
        ...
        if wi == 0 or (wi + 1) % 8 == 0 or wi + 1 == len(records): print(f"learned_transport_v2_eval {wi+1}/{len(records)} sid={sid} sources={len(current)}")

    reports = {name: safe._report(state) for name, state in states.items()}
    so, sm = metric_pair(reports["strong_anchor"]); _, gm = metric_pair(reports["gt_center_rigid"]); _, lm = metric_pair(reports["learned_center_rigid"])
    ...
    print("\n=== P0-F9 LEARNED MOTION TRANSPORT V2 ===")
    print(f"{'variant':24s} {'Overall':>9s} {'Moving':>9s} {'dOverall':>10s} {'dMoving':>9s}")
    for name in VARIANTS:
        o, m = metric_pair(reports[name]); print(f"{name:24s} {o:9.4f} {m:9.4f} {o-so:+10.4f} {m-sm:+9.4f}")
    print("\n=== MOVING BY HORIZON ===")
    for name in VARIANTS:
    ...
    result = {
        "protocol": PROTOCOL, "checkpoint": str(Path(a.checkpoint).resolve()), "checkpoint_epoch": int(ck.get("epoch", -1)),
        "motion_cache": str(Path(a.motion_cache).resolve()), "p0f9_cache": str(Path(a.p0f9_cache).resolve()),
        "existence_threshold": float(a.existence_threshold), "num_windows": len(records), "reports": reports, "diagnostics": diagnostics,
