#!/usr/bin/env python3
"""V19 Innovation v5: true-motion shape supervision + structured endpoints."""
...
from real_motion.v19_innovation import InnovationLossWeights
...
OBJECTIVE_VERSION = "true_motion_responsibility_structured_endpoints_v5"
HEAD_TYPE = "vertical_endpoints_v5"
...
def _evaluate(
    ...
    *,
    ...
    sums = {
        ...
        "bottom_soft_ce": 0.0,
        "top_soft_ce": 0.0,
        "interval_iou_loss": 0.0,
        "vertical_endpoint_loss": 0.0,
    }
    nb = 0
    p_tp = p_fp = p_fn = 0
    sem_ok = sem_n = 0
    bottom_ok = top_ok = geom_n = 0
    z_tp = z_fp = z_fn = 0
    ...
    with torch.inference_mode():
        for raw in _iter_batches(
            ...
            with _autocast(device, amp):
                ...
                _, stats = innovation_endpoint_loss(
                    out,
                    add_target=b["add"],
                    ...
                    interval_iou_weight=float(interval_iou_weight),
                )
            for k in sums:
            ...
            if bool(pos.any()):
                ...
                z_true = (
                    b["vertical"]
                    .permute(0, 1, 3, 4, 2)[pos]
                    .bool()
                )
                Z = int(vertical_bins)
                idx = torch.arange(Z, device=device)[None]
                ...
                bottom_full, top_full = decode_ordered_endpoints(
                    out["bottom_logits"],
                    out["top_logits"],
                )
                bottom_pred = bottom_full[pos]
                top_pred = top_full[pos]
                bottom_ok += int(
                    (bottom_pred == bottom_true).sum().item()
                )
                top_ok += int(
                    (top_pred == top_true).sum().item()
                )
                ...
                z_tp += int((z_pred & z_true).sum().item())
                z_fp += int((z_pred & ~z_true).sum().item())
                z_fn += int((~z_pred & z_true).sum().item())
            else:
                bottom_full, top_full = decode_ordered_endpoints(
                    out["bottom_logits"],
                    out["top_logits"],
    ...
    return {
        **{k: v / max(nb, 1) for k, v in sums.items()},
        ...
        "top_accuracy_on_positive_bev": float(
            top_ok / max(geom_n, 1)
        ),
    ...
    torch.save(
        {
            ...
            "head_type": HEAD_TYPE,
            "training_mode": (
                "innovation_only_frozen_base_true_motion_shape_v5"
            ),
            "epoch": int(epoch),
            ...
            "responsibility_policy": train_index.get(
                "responsibility_policy"
            ),
            "hard_negative_ratio": float(args.hard_negative_ratio),
            "endpoint_sigma": float(args.endpoint_sigma),
            "interval_iou_weight": float(args.interval_iou_weight),
...
def main():
    ...
    p.add_argument("--endpoint-sigma", type=float, default=0.75)
    p.add_argument("--interval-iou-weight", type=float, default=0.5)
    ...
    if (
        ...
        or a.endpoint_sigma < 0
        or a.interval_iou_weight < 0
    ):
    ...
    for name, idx in (("train", train_index), ("val", val_index)):
        if idx.get("positive_mode") != "true_motion_shape":
            raise RuntimeError(
                f"{name} cache must use --positive-mode true_motion_shape"
            )
    overlap = sorted(
    ...
    initial_val = _evaluate(
        ...
        endpoint_sigma=a.endpoint_sigma,
        interval_iou_weight=a.interval_iou_weight,
    )
    preflight = {
        "protocol": PROTOCOL,
        "objective_version": OBJECTIVE_VERSION,
        ...
        "endpoint_sigma": float(a.endpoint_sigma),
        "interval_iou_weight": float(a.interval_iou_weight),
    ...
    print("=== V19 INNOVATION TRUE-MOTION V5 PREFLIGHT ===")
    ...
    for epoch in range(1, int(a.epochs) + 1):
        ...
        for raw in _iter_batches(
            ...
            with _autocast(device, amp):
                ...
                loss, stats = innovation_endpoint_loss(
                    out,
                    add_target=b["add"],
                    semantic_target=b["semantic_target"],
                    vertical_target=b["vertical"],
    ...
    print(f"saved true-motion v5 checkpoints to {out_dir}")
