...
from real_motion.motion_transport_v1.compositor import render_soft_ordered
...
def main():
    ...
    device=a.device if a.device!='cuda' or torch.cuda.is_available() else 'cpu';src=NuScenesWindowSource(dr,info_pkl=info,verbose=False);ds=ManifestDataset(src,mp,'train');pipe=MotionTransportV1(cfg,msp_checkpoint=msp,device=device);names={f.name for f in fields(CausalInputs)};forbidden=[n for n in names if any(x in n.lower() for x in FORBIDDEN)];res={'spec_version':'MT-V1-SPEC-2','manifest_sha256':sha256_file(manifest),'msp':prov,'predict_signature':str(inspect.signature(pipe.predict)),'causal_fields':sorted(names),'forbidden_causal_fields':forbidden,'hard_identity_all':True,'zero_initialized_routed_identity_all':True,'actual_sparse_forward_all':True,'gradient_probe':None,'supervision_audit':{'valid':0,'invalid':0,'observed_valid':0,'unobserved_valid':0,'nonfree_observed_valid':0,'nonfree_unobserved_valid':0},'samples':[]}
    forward_calls=[]
    ...
    gp=res['gradient_probe'];grad_ok=bool(gp and gp['ce_output_nonzero'] and gp['motion_output_nonzero'] and gp['ce_head_nonzero'] and gp['motion_head_nonzero'] and np.isfinite(gp['lambda_safe']) and gp['lambda_safe']>0 and gp['joint_dot_motion_gradient']>0);ok=not forbidden and res['hard_identity_all'] and res['zero_initialized_routed_identity_all'] and res['actual_sparse_forward_all'] and grad_ok;res['pass']=bool(ok);Path(a.output).parent.mkdir(parents=True,exist_ok=True);Path(a.output).write_text(json.dumps(res,indent=2));print(json.dumps(res,indent=2))
