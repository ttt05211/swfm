...
def main():
    ...
    p.add_argument('--cache',required=True);p.add_argument('--empty-latent',required=True)
    p.add_argument('--upstream-ckpt',required=True,help='official occfm_fut epoch=000196.ckpt')
    p.add_argument('--output',required=True);p.add_argument('--window',type=int,default=None);p.add_argument('--max-windows',type=int,default=None);p.add_argument('--batch-size',type=int,default=None);p.add_argument('--steps',type=int,default=None);p.add_argument('--lr',type=float,default=None);p.add_argument('--num-workers',type=int,default=4);p.add_argument('--min-window-coverage',type=float,default=None);p.add_argument('--allow-low-coverage',action='store_true');p.add_argument('--amp',action='store_true')
    a=p.parse_args();cfg=load_runtime_config(a.config,a.override);device='cuda' if torch.cuda.is_available() else 'cpu'
    ...
    if 'traj_encoder.0.weight' not in set(rep.get('loaded_keys',())):raise RuntimeError('tiny trainer requires 12-step OccFM-fut checkpoint (epoch=000196), not the 6-step 199 checkpoint')
    opt=torch.optim.AdamW(model.parameters(),lr=lr,weight_decay=float(get_cfg(cfg,'OPTIMIZATION.TINY.WEIGHT_DECAY',0.01)));scaler=torch.amp.GradScaler('cuda',enabled=a.amp and device=='cuda');planner=WindowPlanner((wh,wh),maxw);step=0;model.train()
    while step<steps:
        before=step
        for batch in dl:
            ...
            if traj is None or tuple(traj.shape[1:])!=(traj_len,2):raise RuntimeError(f'OccFM-fut cache trajectory must be [B,{traj_len},2]')
