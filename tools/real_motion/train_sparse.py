...
def make_transition(window,prior_channels): return MotionWindowFlowMatching(in_channels=16,out_channels=16,model_channels=128,channel_multi=[2,4],input_size=[window,window],trajectory_length=6,init_kernel_size=7,init_3d_conv_channels=64,attn_dim=32,temporal_attn_head=8,spatial_attn_head=8,prior_channels=prior_channels)
def main():
    ...
    while step<a.steps:
        for batch in dl:
            batch={k:(v.to(device,non_blocking=True) if torch.is_tensor(v) else v) for k,v in batch.items()}; planning=batch.get("planning_support",batch["generation_support"]); plan=planner.plan(planning); plan_cov=window_coverage(planning,plan)
            if float(plan_cov.min())<0.95: print(f"[WARN] window planning coverage min={float(plan_cov.min()):.3f}; increase --max-windows or --window before full training")
            hist=crop_windows(batch["moving_history_latent"],plan); fut=crop_windows(batch["future_moving_latent"],plan); sta=crop_windows(batch["static_future_latent"],plan); kta=crop_windows(batch["kta_future_latent"],plan); mask=crop_windows(batch["generation_support"].unsqueeze(2),plan); B,K=hist.shape[:2]; valid=plan.valid.reshape(-1)
