...
def main():
    p=argparse.ArgumentParser();add_config_args(p);p.add_argument('--prepared',required=True);p.add_argument('--vae-ckpt',required=True);p.add_argument('--wm-ckpt',required=True,help='official occfm_fut epoch=000196.ckpt');p.add_argument('--output',required=True);p.add_argument('--max-windows',type=int,default=None);p.add_argument('--vae-mode',choices=['sample','mean'],default=None);p.add_argument('--seed',type=int,default=20260826);p.add_argument('--device',default='cuda');a=p.parse_args();cfg=load_runtime_config(a.config,a.override);mode=a.vae_mode or get_cfg(cfg,'CACHE.VAE_LATENT_MODE','sample');ds=PreparedShardDataset(a.prepared);n=len(ds) if a.max_windows is None else min(len(ds),a.max_windows)
    if n==0:raise RuntimeError('no prepared windows')
    vae,_=load_official_vae(UP,a.vae_ckpt,a.device);wm,_=load_official_wm(UP,a.wm_ckpt,a.device,config_rel=str(get_cfg(cfg,'UPSTREAM.WM_CONFIG','tools/cfgs/occfm_fut.yaml')));ad=OccFMVAEAdapter(vae);branches=('full','static','moving');mm={b:MovingMIoUV2MultiHorizon() for b in branches};overall={b:{h:SemanticIoUAccumulator() for h in REPORT_HORIZONS_S} for b in branches};ts={b:{h:SemanticIoUAccumulator() for h in REPORT_HORIZONS_S} for b in branches}
    for i in range(n):
        s=ds[i];traj=torch.as_tensor(s['trajectory'],dtype=torch.float32)
        if tuple(traj.shape)!=(12,2):raise RuntimeError('P0-A requires OccFM-fut prepared trajectory [12,2]')
        seed=a.seed+i*101;hist={'full':ad.encode(torch.from_numpy(s['full_history_occ']).unsqueeze(0),mode=mode,seed=seed)[0].cpu(),'static':ad.encode(torch.from_numpy(s['static_history_occ']).unsqueeze(0),mode=mode,seed=seed)[0].cpu(),'moving':ad.encode(torch.from_numpy(s['moving_history_occ']).unsqueeze(0),mode=mode,seed=seed)[0].cpu()};zf=ad.encode(torch.from_numpy(s['future_gt_occ']).unsqueeze(0),mode=mode,seed=seed+1)[0].cpu();pred={b:run_frozen_occfm_forecast(wm,hist[b],zf,trajectory=traj,seed=a.seed+i*1009,hist_last=4).numpy() for b in branches}
    ...
    report={'protocol':'P0-A_occfm_fut196_causal_real_motion_decomposition','trajectory_protocol':'occfm_fut_12step_v1','dense_reference':'official_occfm_fut_epoch196','num_windows':n,'vae_mode':mode,'branches':{}}
