#!/usr/bin/env python3
"""Build P0-F4 Strong-W2Det + full-history Sparse-WM latent cache.

Contract
--------
- Frozen Real-Motion MSP keeps the exact P0-F2 Top-2 routing.
- The future source/anchor is the proven occupancy-only strong W2Det contract.
- History conditioning is full native occupancy history, not MOVING/UNCERTAIN only.
- MSP future-motion scores define a causal horizon-wise write support inside the
  selected Top-2 windows.  Expensive future generation still runs only Top-2.
- Frozen VAE uses deterministic FP32 posterior means.
"""
...
from real_motion.nuscenes_adapter import NuScenesWindowSource, WindowTokens, gt_moving_support_for_horizon
...
def main():
    ...
    a = p.parse_args()
    if a.topk != 2:
        raise ValueError("P0-F4 main model is frozen to Top-2")
    ...
    if latent_hw != (50, 50) or window_hw != (20, 20):
        raise RuntimeError("P0-F4 expects 50x50 latent and 20x20 prediction windows")
    ...
    def encoded_samples():
        ...
        for _, _, sid, w in windows:
            ...
            if prepared_count % 25 == 0:
                print(
                    f"P0-F4 prepared {prepared_count}/{len(windows)} "
    ...
    metadata = {
        "protocol": "p0_f4_strong_w2det_full_history_top2_v1",
