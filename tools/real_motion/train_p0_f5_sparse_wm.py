#!/usr/bin/env python3
"""Train P0-F5 anchor-preserving sparse repair flow.

P0-F5 keeps P0-F4 architecture/routing/context unchanged. The only method
correction is the endpoint: the model flows from Strong-W2Det latent to the
frozen-VAE encoding of an occupancy-space sparse repair target. The FM loss is
plain MSE over the whole selected 20x20 latent window; there is no hard latent
MSP loss mask and no auxiliary occupancy/router/preservation loss.
"""
...
def _validate_cache_pair(train_ds, val_ds):
    if train_ds.version != MSP_WM_CACHE_VERSION_V3 or val_ds.version != MSP_WM_CACHE_VERSION_V3:
        raise RuntimeError("P0-F5 requires the v3 occupancy-repair-endpoint cache")
    ...
    for name, meta in (("train", tm), ("val", vm)):
        ...
        if meta.get("loss_contract") != LOSS_CONTRACT:
            raise RuntimeError(f"{name} cache loss contract mismatch")
        if meta.get("target") != "occupancy_sparse_repair_endpoint_vae_latent":
            raise RuntimeError(f"{name} cache target is not the encoded occupancy repair endpoint")
...
def prepare_batch(batch, device):
    """Crop valid Top-2 prediction windows and surrounding full-history context."""
    origins_cpu = batch["window_origins"].long()
    valid_cpu = batch["window_valid"].bool()
    B, K = valid_cpu.shape
    if K != 2:
        raise RuntimeError(f"P0-F5 expects K=2, got {K}")
    ...
    target = crop_windows(batch["repair_target_latent"], plan)
...
def _architecture(args):
    return {
        "protocol": "p0_f5_anchor_preserving_repair_flow_v1",
        ...
        "loss": "plain_full_window_flow_mse_no_latent_mask",
...
def _validate_resume_checkpoint(ck, args, train_ds, val_ds):
    ...
    if arch.get("protocol") != "p0_f5_anchor_preserving_repair_flow_v1":
        raise RuntimeError("resume checkpoint is not P0-F5")
    ...
    if arch.get("loss") != "plain_full_window_flow_mse_no_latent_mask":
...
def main():
    ...
    if a.source_noise_std != 0.0:
        raise ValueError("P0-F5 first run freezes source_noise_std=0")
    ...
    if a.resume_from:
        ...
        print(f"resumed P0-F5 from {a.resume_from} at step={step} best_val={best_val:.6f}")
    ...
    (out / "training_report.json").write_text(
        json.dumps({
            ...
            "decision": "Evaluate Strong W2Det vs trained P0-F5 vs same-support GT repair oracle.",
        }, indent=2),
        encoding="utf-8",
    )
