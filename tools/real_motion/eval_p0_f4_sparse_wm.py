#!/usr/bin/env python3
"""Evaluate P0-F4 Strong-W2Det + full-context Top-2 Sparse World Model."""
...
def _repair_anchor(anchor, proposal, write_bev):
    """Only dynamic semantics may modify the strong anchor inside causal write support."""
    anchor = np.asarray(anchor)
    proposal = np.asarray(proposal)
    write = np.asarray(write_bev, dtype=bool)[..., None]
    if anchor.shape != proposal.shape:
        raise ValueError("anchor/proposal shape mismatch")
    out = anchor.copy()
    anchor_dynamic = np.isin(anchor, DYNAMIC)
    proposal_dynamic = np.isin(proposal, DYNAMIC)
    # Allow the WM to remove a KTA dynamic voxel inside the authorized support,
    # then write predicted dynamic semantics. Non-dynamic anchor content is never
    # regenerated outside this narrow motion-authorized operation.
    out[write & anchor_dynamic] = FREE
    out[write & proposal_dynamic] = proposal[write & proposal_dynamic]
    return out
...
def main():
    ...
    if ds.version != MSP_WM_CACHE_VERSION_V2:
        raise RuntimeError("P0-F4 evaluator requires the v2 strong-anchor cache")
    if int(ds.metadata.get("topk", -1)) != 2:
        raise RuntimeError("P0-F4 evaluator is frozen to Top-2")
    ...
    if not bool(ds.metadata.get("include_eval_payload", False)):
        raise RuntimeError("rebuild validation cache with --include-eval-payload")
    ...
    if arch.get("protocol") != "p0_f4_strong_w2det_full_context_v1":
        raise RuntimeError("Sparse-WM checkpoint is not P0-F4")
    ...
    for i in range(len(ds)):
        ...
        for h, fi in REPORT.items():
            ...
            final = _repair_anchor(anchor, decoded[fi], write_bev[fi])
            _update(model_state, h, final, gt, moving_support[fi])

            oracle = _repair_anchor(anchor, gt, write_bev[fi])
    ...
    report = {
        "protocol": {
            "name": "p0_f4_strong_w2det_full_context_eval_v1",
            "num_windows": len(ds),
            "topk": 2,
            ...
            "fusion": "Strong W2Det is exact default; decoded WM dynamic semantics may write only inside causal MSP horizon support",
        },
