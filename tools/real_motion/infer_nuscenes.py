...
def infer_one(sample,va,model,empty,planner,device,latent_extra_radius=1,seed=0,min_coverage=.95,vae_mode='sample',cfg=None):
    with vae_autocast_context(cfg or {},device):
    ...
    gen_cpu=downsample_support(torch.from_numpy(sample['generation_support_occ']).bool(),(50,50),latent_extra_radius).unsqueeze(0)
    hist_cpu=downsample_support(torch.from_numpy(sample['history_candidate_support']).bool(),(50,50),latent_extra_radius).unsqueeze(0)
    context_cpu=torch.cat([hist_cpu,gen_cpu],1)
    plan_cpu=planner.plan(gen_cpu,context_support=context_cpu)
    coverage=float(window_coverage(gen_cpu,plan_cpu).min());context_cov=float(window_coverage(context_cpu,plan_cpu).mean())
    if coverage<min_coverage:raise RuntimeError(f'future latent support coverage {coverage:.3f} < {min_coverage:.3f}')
    gen=gen_cpu.to(device,non_blocking=True)
    ...
    hist=crop_windows(zh,plan);sta=crop_windows(zs,plan);kta=crop_windows(zk,plan);active=crop_windows(gen.unsqueeze(2),plan)
    B,K=hist.shape[:2];F=zs.shape[1];C=zs.shape[2];valid=plan.valid.reshape(-1)
    ef=empty[None,None].expand(B,F,-1,-1,-1).to(device=device,dtype=zs.dtype);ew=crop_windows(ef,plan)
    g=torch.Generator(device=device);g.manual_seed(int(seed));noise=crop_windows(torch.randn((B,F,C,50,50),device=device,dtype=zs.dtype,generator=g),plan)
    if not bool(valid.any()):full=ef
    else:
        def flat(x):return x.reshape(B*K,*x.shape[2:])[valid]
        fh,fs,fk,fa,fe,fn=map(flat,(hist,sta,kta,active,ew,noise));orig=plan.origins.reshape(B*K,2)[valid]
        traj=torch.as_tensor(sample['trajectory'],device=device,dtype=zs.dtype).unsqueeze(0);traj=traj[:,None].expand(B,K,*traj.shape[1:]).reshape(B*K,*traj.shape[1:])[valid]
        with autocast_context(cfg or {},device):pred=model.sample(fh,tuple(fs.shape[:2])+(C,*planner.window_hw),torch.cat([fs,fk],2),fa,fe,trajectory=traj,window_origins=orig,initial_noise=fn)
    ...
    with vae_autocast_context(cfg or {},device):wm=va.decode_labels(full)[0].cpu()
    ...
    return final,{'window_coverage':coverage,'context_coverage':context_cov,'num_windows':int(plan.valid.sum()),'slot_compute_ratio':int(plan.valid.sum())*planner.window_hw[0]*planner.window_hw[1]/2500.0}
...
def main():
    ...
    (root/'index.json').write_text(json.dumps({'version':'swfm_predictions_v3','vae_mode':mode,'seed':a.seed,'sparse_ckpt':str(Path(a.sparse_ckpt).resolve()),'hardware':cuda_device_summary(device),'entries':entries},indent=2),encoding='utf-8');save_resolved_config(cfg,root/'resolved_config.yaml')
