...
from real_motion.motion_transport_v2 import (
    TARGET_CONTRACT, annotation_map, dynamic_annotations, match_sources_to_annotations, world_points_to_t0,
)
...
PROTOCOL = "p0_f9_v17_local_stwm_rigid_transport_eval_v1"
VARIANTS = ("strong_anchor", "local_stwm_center_always", "local_stwm_center_rigid", "gt_center_rigid")


def load_cache(path):
    obj=torch.load(path,map_location="cpu",weights_only=False)
    if obj.get("version")!=LOCAL_STWM_V17_CACHE_VERSION: raise RuntimeError("V17 cache version mismatch")
    meta=obj.get("metadata") or {}
    if meta.get("target_contract")!=TARGET_CONTRACT: raise RuntimeError("target contract mismatch")
    if meta.get("representation_contract")!=REPRESENTATION_CONTRACT: raise RuntimeError("representation contract mismatch")
    records=obj.get("records") or []
    if not records: raise RuntimeError("V17 cache has no records")
    return meta,records


def load_model(path,device):
    ck=torch.load(path,map_location="cpu",weights_only=False)
    if ck.get("protocol")!=MODEL_PROTOCOL_V17: raise RuntimeError(f"checkpoint protocol mismatch: {ck.get('protocol')}")
    cfg=config_from_mapping_v17(ck.get("model_config")); model=LocalSpatialTemporalWorldModelV17(cfg).to(device)
    model.load_state_dict(ck["state_dict"],strict=True); model.eval(); return ck,model


def window_from_record(r):
    return WindowTokens(scene_name=str(r["scene_name"]),history_tokens=tuple(str(x) for x in r["history_tokens"]),
...
def metric_pair(report): return float(report["overall"]["mIoU"]),float(report["moving"]["mIoU"])
...
def main():
    ...
    p.add_argument("--checkpoint",required=True); p.add_argument("--dataroot",required=True); p.add_argument("--info-pkl",required=True)
    p.add_argument("--output",required=True); p.add_argument("--existence-threshold",type=float,default=0.5)
    p.add_argument("--match-max-distance-m",type=float,default=4.0); p.add_argument("--max-windows",type=int,default=0)
    ...
    ds=MSPWorldModelCacheDataset(a.p0f9_cache); sid_to_idx={str(e["sample_id"]):i for i,e in enumerate(ds.entries)}
    missing=[str(r["sample_id"]) for r in records if str(r["sample_id"]) not in sid_to_idx]
    if missing: raise RuntimeError(f"P0-F9 cache misses V17 samples: {missing[:5]}")
    if a.max_windows>0: records=records[:min(len(records),a.max_windows)]
    device=torch.device(a.device if a.device!="cuda" or torch.cuda.is_available() else "cpu")
    ck,model=load_model(a.checkpoint,device); use_rep=bool(ck.get("use_representation",False))
    source=NuScenesWindowSource(a.dataroot,info_pkl=a.info_pkl,verbose=False)
    strong_cfg=StrongW2DetConfig(free_label=int(pcfg.free_label)); states={name:safe._new_metrics() for name in VARIANTS}
    kta_errors=[]; learned_errors=[]; kta_fde=[]; learned_fde=[]
    exist_tp=exist_fp=exist_fn=exist_correct=exist_total=0; source_count=supervised_count=0

    for wi,rec in enumerate(records):
        sid=str(rec["sample_id"]); sample=ds[sid_to_idx[sid]]; payload=safe._sample_payload(sample,torch.device("cpu")); w=window_from_record(rec)
        ...
        if len(current)!=int(rec["features"].shape[0]): raise RuntimeError(f"{sid}: Strong source count differs from V17 cache")
        ...
        features=rec["features"].float().to(device); tube=rec["local_semantic_tube"].to(device)
        ...
        pred_residual=out["residual_xy_m"].float().cpu().numpy(); exist_prob=torch.sigmoid(out["existence_logits"].float()).cpu().numpy()
        ...
        for i in range(len(current)):
            ...
            for h in ids:
                kta_errors.append(float(np.linalg.norm(target_res[i,h]))); learned_errors.append(float(np.linalg.norm(pred_residual[i,h]-target_res[i,h])))
            if len(ids):
            ...
            if supervised[i]:
                ge=rec["existence"][i].numpy()>0.5; pe=exist_prob[i]>=a.existence_threshold
                exist_correct+=int((ge==pe).sum()); exist_total+=FUTURE_FRAMES
                exist_tp+=int((ge&pe).sum()); exist_fp+=int((~ge&pe).sum()); exist_fn+=int((ge&~pe).sum())

        anns0=dynamic_annotations(source.nusc,w.t0_token); source_tokens=match_sources_to_annotations(current,anns0,max_distance_m=a.match_max_distance_m)
        future_maps=[annotation_map(source.nusc,tok) for tok in w.future_tokens]; ann0_by_token={str(x["instance_token"]):x for x in anns0}; t0_pose=history_poses[-1]
        for horizon,hi in safe.REPORT.items():
            ...
            baseline_all=[]; learned_all=[]; learned_exist=[]; oracle_baseline=[]; oracle_repl=[]; dt=(hi+1)*float(pcfg.frame_dt_s)
            for i,comp in enumerate(current):
                src_center=np.asarray(comp["centroid_world"],dtype=np.float64); v=np.asarray(velocities.get(i,np.zeros(3)),dtype=np.float64)
                base=rasterize_rigid_component(comp["voxel_indices"],int(comp["class_id"]),t0_pose,future_poses[hi],
                    source_center_world=src_center,target_center_world=src_center+v*dt,yaw_delta_rad=0.0,grid=pcfg.grid)
                ...
                repl=rasterize_rigid_component(comp["voxel_indices"],int(comp["class_id"]),t0_pose,future_poses[hi],
                    source_center_world=src_center,target_center_world=center_pred,yaw_delta_rad=0.0,grid=pcfg.grid)
                learned_all.append(repl)
                if exist_prob[i,hi]>=a.existence_threshold: learned_exist.append(repl)
                ...
                if token is not None:
                    ...
                    if ann0 is not None and annh is not None:
                        ...
                        gd=np.asarray(annh["center_world"],dtype=np.float64)-np.asarray(ann0["center_world"],dtype=np.float64)
                        oracle_repl.append(rasterize_rigid_component(comp["voxel_indices"],int(comp["class_id"]),t0_pose,future_poses[hi],
                            source_center_world=src_center,target_center_world=src_center+gd,yaw_delta_rad=0.0,grid=pcfg.grid))
        ...
        if wi==0 or (wi+1)%8==0 or wi+1==len(records): print(f"v17_local_stwm_eval {wi+1}/{len(records)} sid={sid} sources={len(current)}")

    reports={name:safe._report(state) for name,state in states.items()}; so,sm=metric_pair(reports["strong_anchor"]); _,gm=metric_pair(reports["gt_center_rigid"]); _,lm=metric_pair(reports["local_stwm_center_rigid"])
    ...
    diagnostics={"source_count":source_count,"supervised_source_count":supervised_count,"source_supervision_fraction":supervised_count/max(source_count,1),
        ...
        "existence_accuracy":exist_correct/max(exist_total,1),"existence_precision":prec,"existence_recall":rec_e,"existence_f1":2*prec*rec_e/max(prec+rec_e,1e-12),
        "moving_oracle_recovery_fraction":recovery}
    print("\n=== P0-F9 V17 LOCAL SPATIAL-TEMPORAL WORLD MODEL ==="); print(f"variant={ck.get('variant')} checkpoint_epoch={ck.get('epoch')} use_representation={use_rep} overlap_weight={ck.get('overlap_weight')}")
    ...
    result={"protocol":PROTOCOL,"checkpoint":str(Path(a.checkpoint).resolve()),"checkpoint_epoch":int(ck.get("epoch",-1)),"variant":ck.get("variant"),
        "use_representation":use_rep,"overlap_weight":float(ck.get("overlap_weight",0.0)),"local_stwm_cache":str(Path(a.local_stwm_cache).resolve()),
        "p0f9_cache":str(Path(a.p0f9_cache).resolve()),"num_windows":len(records),"reports":reports,"diagnostics":diagnostics,
        "model_config":ck.get("model_config"),"target_contract":TARGET_CONTRACT,"representation_contract":REPRESENTATION_CONTRACT,"cache_metadata":cache_meta}
