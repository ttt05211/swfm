#!/usr/bin/env python3
"""Evaluate P0-F5 Strong-W2Det + sparse occupancy-repair-endpoint World Model."""
...
def main():
    ...
    if ds.version != MSP_WM_CACHE_VERSION_V3:
        raise RuntimeError("P0-F5 evaluator requires the v3 occupancy-repair-endpoint cache")
    if int(ds.metadata.get("topk", -1)) != 2:
        raise RuntimeError("P0-F5 evaluator is frozen to Top-2")
    ...
    if arch.get("protocol") != "p0_f5_anchor_preserving_repair_flow_v1":
        raise RuntimeError("Sparse-WM checkpoint is not P0-F5")
    ...
    report = {
        "protocol": {
            "name": "p0_f5_anchor_preserving_repair_flow_eval_v1",
            "num_windows": len(ds),
            "topk": 2,
