...
def main():
    ...
    warm=int(get(cfg,'training.profile_warmup_microsteps',50));measure=int(get(cfg,'training.profile_measure_microsteps',200));first=a.force_checkpointing=='on'
    try:pipe,raw,opt,prof=_pass(cfg,src,train_ds,ctx,lambda_ref,first,warm,measure,seed)
    except torch.cuda.OutOfMemoryError as e:
        raise RuntimeError('profile OOM before memory-threshold decision; rerun all ranks with --force-checkpointing on') from e
    ...
    acc=int(get(cfg,'training.accumulation_steps',4));local_len=int(math.ceil(len(train_ds)/ctx.world_size));groups=int(math.ceil(local_len/acc));quarter=max(1,int(math.ceil(groups/4)));quarter_saves=sum(1 for gi in range(groups) if (gi+1)%quarter==0 or gi+1==groups);saves_per_epoch_worst=quarter_saves+3;train_epoch_core=prof['mean_wall_s_per_local_scene']*local_len;epoch_total=train_epoch_core+dev_time+saves_per_epoch_worst*save_time;full=a.dev_max_windows is None;safety=max(60.,2.*prof['p95_group_s']);final_reserve=dev_time+2.*save_time+safety;budget=float(get(cfg,'training.max_hours',4))*3600;E=int(math.floor(max(0,budget-final_reserve)/epoch_total)) if epoch_total>0 and full else 0;locked=copy.deepcopy(cfg);locked['loss']['lambda_reference']=float(lambda_ref);locked['network']['activation_checkpointing']=bool(prof['checkpointing']);locked['training']['epochs_locked']=E if E>0 else None;locked['training']['wall_clock_final_reserve_seconds']=float(final_reserve);locked['training']['wall_clock_next_group_guard_seconds']=float(max(prof['p95_group_s']*1.5,1.));locked.setdefault('runtime',{}).update({'code_commit':_git(),'profile_formal_full_dev':full,'profile_world_size':ctx.world_size,'profile_group_p95_s':prof['p95_group_s'],'profile_dev_s':float(dev_time),'profile_full_checkpoint_s':save_time,'profile_safety_s':float(safety),'manifest_sha256':sha256_file(manifest),'msp_sha256':sha256_file(msp)});result={'spec_version':'MT-V1-SPEC-2','formal_full_dev':full,'world_size':ctx.world_size,'train_windows':len(train_ds),'dev_windows':len(dev_ds),'lambda_ref':lambda_ref,'gradient_calibration':{'probe_epsilon':float(get(cfg,'loss.gradient_calibration.probe_epsilon',1e-3)),'max_ce_antagonistic_fraction_of_motion':float(get(cfg,'loss.gradient_calibration.max_ce_antagonistic_fraction_of_motion',.5)),'rows':calrows},'msp':msp_checkpoint_provenance(msp),'profile':prof,'checkpointing_reran_after_memory_threshold':reran,'timing':{'estimated_train_epoch_core_s':train_epoch_core,'measured_dev_s':dev_time,'measured_full_checkpoint_s':save_time,'quarter_saves_per_epoch':quarter_saves,'worst_full_saves_per_epoch':saves_per_epoch_worst,'estimated_epoch_total_s':epoch_total,'wall_clock_final_reserve_s':final_reserve,'wall_clock_next_group_guard_s':locked['training']['wall_clock_next_group_guard_seconds'],'max_budget_s':budget},'epochs_locked':locked['training']['epochs_locked'],'dev_timing_report':dev_report}
    if ctx.is_main:
        (od/'profile.json').write_text(json.dumps(result,indent=2));(od/'resolved_profile_config.yaml').write_text(yaml.safe_dump(locked,sort_keys=False));print(json.dumps({k:result[k] for k in ('lambda_ref','profile','timing','epochs_locked')},indent=2))
        if full and E<=0:raise RuntimeError('4h budget leaves no complete epoch after full data/dev/checkpoint timing')
    barrier(ctx)
