...
def main():
    p=argparse.ArgumentParser();p.add_argument('--config',default=None);p.add_argument('--override',action='append',default=[]);p.add_argument('--output-dir',default=None);p.add_argument('--dev-max-windows',type=int,default=None);p.add_argument('--force-checkpointing',choices=('auto','on','off'),default='auto');p.add_argument('--formal-server-gate',action='store_true');a=p.parse_args();cfg=load_config(a.config,a.override);ctx=init_distributed(require_cuda=a.formal_server_gate)
    if ctx.world_size>2:raise RuntimeError('SPEC-2 supports max 2 GPUs')
    formal_env=formal_server_environment(ctx,cfg) if a.formal_server_gate else {'torch':torch.__version__,'cuda_build':torch.version.cuda,'world_size':ctx.world_size,'device':str(ctx.device)}
    if a.formal_server_gate and a.dev_max_windows is not None:raise RuntimeError('formal server profile requires full dev; remove --dev-max-windows')
    ...
    if a.formal_server_gate and (warm!=50 or measure!=200):raise RuntimeError(f'formal server profile requires exactly 50 warmup + 200 measured microsteps, got {warm}+{measure}')
