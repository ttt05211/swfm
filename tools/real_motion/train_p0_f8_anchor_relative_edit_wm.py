#!/usr/bin/env python3
"""Train P0-F8 anchor-relative sparse innovation World Model.

P0-F8 returns to the P0-F6 uniform FM objective and changes the decoder-aware
supervision from absolute future semantics to deployment-aligned edit actions:
KEEP / CLEAR / WRITE(dynamic class).  Every EDIT voxel is retained and hard KEEP
examples are balanced against it.  A Lovasz-Softmax term is computed on the
semantic occupancy implied by the actions, so the auxiliary objective is closer
to final IoU while KEEP remains an explicit safe action.

Loss:
    L = L_FM + lambda_edit * (CE_action + beta_lovasz * Lovasz_result)
"""
...
def _validate_edit_pair(train_edit, val_edit, train_ds, val_ds) -> None:
    ...
    for name, edit in (("train", train_edit), ("val", val_edit)):
        m = edit.metadata
        if m.get("protocol") != EDIT_PROTOCOL:
        ...
        if m.get("target_contract") != "exact_strong_anchor_relative_keep_clear_write_inside_causal_msp_support":
...
def _build_optimizer(model, reuse, *, lr: float, backbone_lr_scale: float, weight_decay: float):
    ...
    if not new_params:
        raise RuntimeError("P0-F8 found no new trainable edit-head parameters")
    groups = []
    if pretrained:
        groups.append({
            "params": pretrained,
...
def edit_loss_for_endpoint(
    ...
    *,
    ...
    gen.manual_seed(int(selection_seed))
    selected = [
        select_balanced_edit_supervision(
            rec,
    ...
    indices = [x["flat_indices"].to(endpoint_full.device) for x in selected]
...
def _architecture(args, edit_lambda, optimizer_summary, train_ds):
    return {
        ...
        "edit_head_input": "frozen_decoder_dynamic_semantic_logprobs+exact_anchor_slot+horizon_embedding",
        "edit_sampling": {
            "all_edit_voxels": True,
            "keep_ratio": float(args.keep_ratio),
            "keep_when_no_edit": int(args.keep_when_no_edit),
            "keep_priority": "true-moving correct dynamic > correct dynamic > background near edit",
        },
        ...
        "edit_lambda_mode": "explicit" if args.edit_lambda is not None else "first_batch_gradient_calibration",
        "keep_bias": float(args.keep_bias),
        "optimizer_groups": optimizer_summary,
        "repair_endpoint_contract": f6.REPAIR_CONTRACT,
        "deployment": "exact_Strong_W2Det_outside_support; KEEP/CLEAR/WRITE_only_inside_causal_MSP_support",
        "checkpoint_selection": "edit_aware_validation_objective_but_all_validation_steps_are_saved",
...
def _payload(
    *, model_state, optimizer, step, best_val, history, train_ds, val_ds,
    train_edit, val_edit, args, upstream_ckpt, reuse, optimizer_summary,
    skipped_train_batches, edit_lambda, calibration,
):
    return {
...
def _validate_resume(ck, args, train_ds, val_ds, train_edit, val_edit):
    ...
    checks = (
        ...
        (float(arch.get("edit_sampling", {}).get("keep_ratio", -1.0)), float(args.keep_ratio), "keep-ratio"),
        (float(arch.get("optimizer_groups", {}).get("backbone_lr_scale", -1.0)), float(args.backbone_lr_scale), "backbone-lr-scale"),
...
def main():
    ...
    p.add_argument("--edit-lambda", type=float, default=None)
    p.add_argument("--edit-grad-ratio", type=float, default=0.5)
    p.add_argument("--edit-lambda-min", type=float, default=1e-4)
    p.add_argument("--edit-lambda-max", type=float, default=10.0)
    ...
    if not 0.0 <= a.backbone_lr_scale <= 1.0:
        raise ValueError("backbone-lr-scale must be in [0,1]")
    if a.keep_ratio < 0 or a.keep_when_no_edit < 0 or a.lovasz_weight < 0:
        raise ValueError("keep/lovasz settings must be non-negative")
    if a.edit_lambda is not None and a.edit_lambda <= 0:
        raise ValueError("explicit edit lambda must be positive")
    if a.edit_grad_ratio <= 0:
    ...
    _validate_training_cache(train_ds, min_train_windows=int(a.min_train_windows))

    train_edit = EditTargetCache(a.train_edit_targets)
    val_edit = EditTargetCache(a.val_edit_targets)
    ...
    edit_lambda = float(a.edit_lambda) if a.edit_lambda is not None else None
    calibration = None

    if a.resume_from:
        ck = torch.load(a.resume_from, map_location="cpu", weights_only=False)
        _validate_resume(ck, a, train_ds, val_ds, train_edit, val_edit)
        ...
        print(
            f"resumed P0-F8 from {a.resume_from} at step={step} "
    ...
    while step < a.steps:
        ...
        with torch.autocast(
            ...
            edit_loss, edit_info = edit_loss_for_endpoint(
                ...
                vae=vae,
                keep_ratio=float(a.keep_ratio),
                keep_when_no_edit=int(a.keep_when_no_edit),
                lovasz_weight=float(a.lovasz_weight),
                deterministic=False,
                selection_seed=int(a.seed) + int(step) * 1000003,
            )
        ...
        if not torch.isfinite(fm_loss) or not torch.isfinite(edit_loss):
            raise RuntimeError(f"non-finite P0-F8 loss at step {step}: fm={fm_loss} edit={edit_loss}")

        if edit_lambda is None:
            edit_lambda, calibration = f6.calibrate_semantic_lambda(
                fm_loss,
                edit_loss,
                model,
                target_ratio=float(a.edit_grad_ratio),
                lambda_min=float(a.edit_lambda_min),
                lambda_max=float(a.edit_lambda_max),
            )
            calibration.update({
