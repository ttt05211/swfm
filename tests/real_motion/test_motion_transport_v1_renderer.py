...
from real_motion.motion_transport_v1.compositor import render_soft_ordered,compose_hard,hard_kta_identity,soft_argmax_scene,predicted_source_points_world
...
def fixture(shape=(10,10,4),origin=(-2,-2,-.8),source_origin=(4,4),device='cpu'):
...
def _targets_for_delta(g,c,d,td):
    gt=compose_hard(c,d,td,[0],grid=g);src=d.sources[0];ids=np.arange(len(src.points_world));td_cpu=td.detach().float().cpu();gxy=np.stack([predicted_source_points_world(src,float(h),td_cpu[0,i].numpy(),c.history_ego_to_world[-1])[:,:2] for i,h in enumerate(d.horizons_s)]);return gt,TrainingTargets(gt,np.ones_like(gt,bool),{0:MotionTarget(0,np.ones(6,bool),ids,gxy)})
def _calibrated_lambda(g,c,d,targets,device='cpu'):
    base=torch.zeros((1,6,3),device=device,requires_grad=True);ratios=[];rows=[]
...
def _optimize(g,c,d,targets,lam,steps=100,use_schedule=False,device='cpu'):
    delta=torch.zeros((1,6,3),device=device,requires_grad=True);opt=torch.optim.Adam([delta],lr=.01);gt=targets.future_semantics;before=np.count_nonzero(compose_hard(c,d,delta,[0],grid=g)!=gt)
...
def test_soft_probability_gradient_and_chunk_equivalence():
    ...
    for x,y in zip(sa.horizons,sb.horizons):assert torch.equal(x.flat_indices,y.flat_indices) and torch.all(x.probabilities>=0) and torch.all(x.probabilities<=1) and torch.allclose(x.probabilities.sum(1),torch.ones(len(x.probabilities)),atol=1e-5) and torch.allclose(x.probabilities,y.probabilities,atol=1e-7,rtol=1e-6)
...
def test_soft_zero_probability_and_ce_gradient_are_finite_difference_consistent():
    g,c,d=fixture();td=torch.zeros((1,6,3));td[:,:,0]=.35;td[:,:,1]=-.35;_,targets=_targets_for_delta(g,c,d,td);x=torch.zeros((1,6,3),requires_grad=True)
    def f(v):
        scene=render_soft_ordered(c,d,v,[0],grid=g)
        for row in scene.horizons:
            assert torch.isfinite(row.probabilities).all();assert torch.all(row.probabilities>=0) and torch.all(row.probabilities<=1);assert torch.allclose(row.probabilities.sum(1),torch.ones(len(row.probabilities)),atol=1e-5)
        loss,_=occupancy_ce_full(scene,targets);return loss
    y=f(x);grad=torch.autograd.grad(y,x)[0];eps=1e-4
    for j in (0,1,2):
        xp=x.detach().clone();xm=x.detach().clone();xp[0,0,j]+=eps;xm[0,0,j]-=eps;fd=float((f(xp)-f(xm))/(2*eps));assert np.isfinite(fd);assert np.isclose(float(grad[0,0,j]),fd,rtol=.05,atol=5e-3),(j,float(grad[0,0,j]),fd)
def test_hard_zero_selected_delta_is_exact_kta_identity():
    g,c,d=fixture();zero=torch.zeros((1,6,3));hard_zero=compose_hard(c,d,zero,[0],grid=g);kta=hard_kta_identity(c,d,grid=g);assert np.array_equal(hard_zero,kta)
...
def _run_device_acceptance(device):
    g,c,d=fixture();cases=[(.35,0,.08),(-.35,0,-.08),(0,.35,.08),(0,-.35,-.08),(.35,.35,0),(-.35,.35,0),(.35,-.35,0),(-.35,-.35,0),(0,0,.8),(0,0,-.8)]
    for dx,dy,yaw in cases:
        td=torch.zeros((1,6,3),device=device);td[:,:,0]=dx;td[:,:,1]=dy;td[:,:,2]=yaw;_,targets=_targets_for_delta(g,c,d,td);lam,_,_=_calibrated_lambda(g,c,d,targets,device=device);before,after,delta,_=_optimize(g,c,d,targets,lam,100,use_schedule=True,device=device);assert before>0 and after<before,(device,dx,dy,yaw,before,after,lam,delta[0,0].tolist())
def test_complete_direction_yaw_schedule_acceptance_cpu():_run_device_acceptance('cpu')
def test_complete_direction_yaw_schedule_acceptance_cuda_if_available():
    if not torch.cuda.is_available():return
    _run_device_acceptance('cuda')
