...
def test_joint_calibrated_reachability_improves_hard():
    g,c,d=fixture();td=torch.zeros((1,6,3));td[:,:,0]=.35;td[:,:,2]=.08;gt=compose_hard(c,d,td,[0],grid=g);src=d.sources[0];ids=np.arange(len(src.points_world));gxy=np.stack([predicted_source_points_world(src,float(h),td[0,i].numpy(),c.history_ego_to_world[-1])[:,:2] for i,h in enumerate(d.horizons_s)]);targets=TrainingTargets(gt,np.ones_like(gt,bool),{0:MotionTarget(0,np.ones(6,bool),ids,gxy)})
    # Exact zero can be a symmetric soft-renderer point on some PyTorch versions.  Use a
    # tiny causal probe only to estimate the locked gradient scale, then verify recovery
    # from the actual zero-initialized deployment state.
    probe=torch.zeros((1,6,3),requires_grad=True);probe.data[:,:,0]=1e-3;probe.data[:,:,2]=1e-3;ps=render_soft_ordered(c,d,probe,[0],grid=g);po,_=occupancy_ce_full(ps,targets);pmn,pn,_=motion_loss_sum(probe,[0],d,targets,c.history_ego_to_world[-1]);pm=pmn/pn;go=torch.autograd.grad(po,probe,retain_graph=True)[0];gm=torch.autograd.grad(pm,probe)[0];Go=float(go.norm());Gm=float(gm.norm());cos=float(torch.dot(go.reshape(-1),gm.reshape(-1))/(go.norm()*gm.norm()).clamp_min(1e-12));lam=calibrated_gradient_ratio(Go,Gm,cos,max_ce_antagonistic_fraction_of_motion=.5);assert np.isfinite(lam) and lam>0
    delta=torch.zeros((1,6,3),requires_grad=True);opt=torch.optim.Adam([delta],lr=.01);before=np.count_nonzero(compose_hard(c,d,delta,[0],grid=g)!=gt)
    for _ in range(100):
