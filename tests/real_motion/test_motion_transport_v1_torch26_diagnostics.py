...
from real_motion.motion_transport_v1.losses import occupancy_ce_full,motion_loss_sum,calibration_probe_like,lambda_at

def test_torch26_reachability_diagnostics():
    g,c,d=fixture();cases=[(.35,0,.08,False),(-.35,0,-.08,False),(0,.35,.08,False),(0,-.35,-.08,False),(.35,.35,0,False),(-.35,.35,0,False),(.35,-.35,0,False),(-.35,-.35,0,False),(0,0,.8,True),(0,0,-.8,True)];rows=[];failed=[]
    for dx,dy,yaw,use_schedule in cases:
        ...
        for step in range(100):
            opt.zero_grad();scene=render_soft_ordered(c,d,delta,[0],grid=g);occ,_=occupancy_ce_full(scene,targets);mn,n,_=motion_loss_sum(delta,[0],d,targets,c.history_ego_to_world[-1]);weight=lambda_at(step/99.,lam) if use_schedule else lam;(occ+weight*mn/n).backward();opt.step()
            if step in (0,9,24,49,74,99):snap[str(step+1)]={'lambda':weight,'delta_h0':delta.detach()[0,0].tolist(),'grad':_grad_row(g,c,d,targets,delta.detach())}
        after=int(np.count_nonzero(compose_hard(c,d,delta,[0],grid=g)!=gt));state=opt.state[delta];row={'target':[dx,dy,yaw],'scheduled':use_schedule,'before':before,'after':after,'lambda_ref':lam,'zero':_grad_row(g,c,d,targets,zero),'probe':_grad_row(g,c,d,targets,probe),'one_sided_dx':_one_sided_ce(g,c,d,targets,0),'one_sided_dy':_one_sided_ce(g,c,d,targets,1),'snapshots':snap,'adam_exp_avg_h0':state['exp_avg'][0,0].tolist(),'adam_exp_avg_sq_h0':state['exp_avg_sq'][0,0].tolist()};rows.append(row)
