...
def test_free_is_never_confident_static():
    # Non-eligible class 15 is deterministic background even with short evidence.
    x=np.full((3,4,4,2),17,dtype=np.int64);x[:,1,1,0]=15;m=decompose_masks(x,PersistenceMotionConfig());assert m.confident_static[1,1,0];assert not m.confident_static[0,0,0];assert m.free[0,0,0]
...
def test_persistent_eligible_object_is_uncertain_not_hard_static():
    # A parked car may start in the future. It is not MOVING now, but must
    # retain generator write permission rather than being hard protected.
    x=np.full((3,8,8,2),17,dtype=np.int64);x[:,3:5,3:5,0]=4;m=decompose_masks(x,PersistenceMotionConfig());cur=(x[-1]==4);assert m.uncertain[cur].all();assert not m.confident_static[cur].any();assert not m.moving[cur].any()
...
def test_unobserved_holes_do_not_change_background_static_route():
...
def test_one_observation_eligible_thing_stays_uncertain():
    x=np.full((4,6,6,1),17,dtype=np.int64)
    x[-1,2,2,0]=4
    obs=np.zeros_like(x,dtype=bool)
    obs[-1,2,2,0]=True
...
def test_noneligible_stuff_centroid_shift_is_background_static():
    ...
    assert not m.uncertain[cur].any()
    assert m.confident_static[cur].all()


def test_low_persistence_eligible_thing_is_uncertain_not_moving():
...
def test_prepare_kta_uses_only_moving_while_uncertain_is_zero_motion_anchor():
    from real_motion.prepared import PrepareConfig,prepare_nuscenes_window
    from real_motion.nuscenes_adapter import WindowTokens
    class Source:
        def load_semantics(self,scene,token):
            x=np.full((6,6,1),17,dtype=np.int64)
            # parked car stays at x=1; moving car shifts x=2 -> x=3
            if token=='h0':
                x[1,1,0]=4;x[2,4,0]=4
            else:
                x[1,1,0]=4;x[3,4,0]=4
    ...
    g=OccupancyGrid(-3,-3,-1,(1,1,1),(6,6,1))
    cfg=PrepareConfig(history_frames=2,future_frames=2,frame_dt_s=1.0,endpoint_tube_radii=(0,0),swept_tube_radii=(0,0),uncertain_tube_radii=(0,0),trajectory_length=4,trajectory_hist_last=2,trajectory_zero_prefix=0,trajectory_protocol='unit_4step',require_temporal_info=False,grid=g,motion=PersistenceMotionConfig(min_observed_frames=2,min_static_observations=2,history_dt_s=1.0,voxel_size_xy_m=(1,1),moving_speed_mps=0.5),kta=KTAConfig(history_dt_s=1.0,max_match_distance_m=2.0))
    out=prepare_nuscenes_window(src,w,cfg,include_gt=False)
    # parked eligible thing is uncertain and remains at x=1 under zero motion
    assert out['uncertain_zero_future_occ'][0,1,1,0]==4
    # moving thing alone is extrapolated by KTA from x=3 to x=4 at 1s
    assert out['moving_kta_future_occ'][0,4,4,0]==4
    assert out['moving_kta_support'][0,4,4]
    assert not out['moving_kta_support'][0,1,1]
