...
def test_model_loss_handles_padding_and_an_empty_window():
    batch = collate_probe_records([_record(2), _record(0)])
    model = MSPProbeHead(
        feature_dim=FEATURE_DIM, hidden_dim=32, num_heads=4,
        num_modes=2, future_frames=H, dropout=0.0,
    )
    out = model(batch["features"], batch["candidate_mask"])
    assert out["activation_logits"].shape == (2, 2, H)
    assert out["mu_residual_xy_m"].shape == (2, 2, H, 2, 2)
    loss, info = msp_probe_loss(out, batch)
    assert torch.isfinite(loss)
    assert info["num_activation_labels"] == 2 * H
    assert info["num_location_labels"] == 2 * H
